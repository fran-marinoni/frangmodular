...
import foto2 from "@/assets/2. ESTACIONES/6. ERGOFLEX/1. SIMPLE UP/2.webp";
import fotoDSC1 from "@/assets/2. ESTACIONES/6. ERGOFLEX/1. SIMPLE UP/DSC09012 copy.webp";
import fotoDSC2 from "@/assets/2. ESTACIONES/6. ERGOFLEX/1. SIMPLE UP/DSC09016 copy.webp";

import fotoUp1 from "@/assets/2. ESTACIONES/6. ERGOFLEX/3. UP OPERATIVE/DSC_1334.webp";
import fotoUp2 from "@/assets/2. ESTACIONES/6. ERGOFLEX/3. UP OPERATIVE/DSC_1338.webp";

import fotoRoundA from "@/assets/2. ESTACIONES/6. ERGOFLEX/5. ROUND/A.webp";
import fotoRoundB from "@/assets/2. ESTACIONES/6. ERGOFLEX/5. ROUND/B.webp";

const heroPhoto = fotoDSC1;
const configImages = [
  { src: foto0, label: "Simple Up" },
  { src: foto1, label: "Simple Up" },
  { src: fotoRoundA, label: "Round" },
  { src: fotoUp1, label: "Up Operative" },
];
const ambientPhotos = [fotoDSC2, fotoUp2, fotoRoundB];
...
const EstacionErgoflex = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead title="Ergoflex – Estación de trabajo | Generación Modular" description="Ergoflex ofrece estaciones ergonómicas con altura ajustable para un trabajo saludable y productivo." canonical="/estaciones/ergoflex" />
      <Header />
        ...
        <section className="relative">
          <div className="grid grid-cols-1 md:grid-cols-2 w-full min-h-[85vh]">
            <div className="relative overflow-hidden min-h-[50vh] md:min-h-0">
              <img src={heroPhoto} alt="Estación Ergoflex" className="w-full h-full object-cover absolute inset-0" />
            </div>
            <div className="flex flex-col">
              <div className="px-6 py-8 md:py-12 flex-1 flex flex-col justify-center">
                <h1 className="font-display text-6xl md:text-7xl lg:text-8xl font-black tracking-tight leading-none mb-4">Ergoflex.</h1>
                <p className="text-xs leading-relaxed text-muted-foreground max-w-sm mb-6">
                  Ergoflex ofrece estaciones ergonómicas con altura ajustable. Incluye líneas Simple Up, Up Operative, Mesa Vix y Round para adaptarse a cualquier necesidad.
                </p>
              </div>
                    ...
                    {openAccordion === item && (
                      <div className="px-6 pb-4">
                        <p className="text-xs text-muted-foreground">
                          {item === "Colores disponibles" ? "Consulta con nuestro equipo las opciones de color disponibles." : `Información sobre ${item.toLowerCase()} del producto Ergoflex.`}
                        </p>
                      </div>
                    )}
                  </div>
        ...
        <section className="border-t border-border">
          <div className="grid grid-cols-1 md:grid-cols-2 min-h-[70vh]">
            <div className="relative overflow-hidden min-h-[50vh] md:min-h-0 bg-muted">
              <img src={fotoDSC2} alt="Espacio Ergoflex" className="w-full h-full object-cover absolute inset-0" />
...
export default EstacionErgoflex;
